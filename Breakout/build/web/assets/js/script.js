$(function(){
    on();
   socket = new WebSocket("ws://localhost:8080/Breakout/endpoint");
   ...
   window.addEventListener('keydown',function(e){
        if (e.keyCode === 13) {
            if(hastoload){on(); hastoload = false;}
            socket.send("start");
        }
    });
   socket.onopen = Startup;
   checkKey();
});
var keyState = {};
var socket;
...
var checkKey = function() {
    ...
    if(keyState[leftkeycode]){
        socket.send("moveleft");
    }
    if(keyState[rightkeycode]){
...
var showpressenter = function(){
    ...
    var c = document.getElementById("gameCanvas");
                ...
                img.onload = function(){
                    off();
                    ctx.drawImage(img,280,300,395,46);};
};
...
var showtext = function(data){
    ...
    c.height=668;
    var ctx = c.getContext("2d");
    ctx.font = "45px Lucida Calligraphy";
    ctx.fillStyle = "white";
    ...
    if (hastoload == true){
                var img = new Image();
                img.src = "/Breakout/assets/images/enter-key.png";
                img.onload = function(){
                    ctx.drawImage(img,825,290,50,50);};
    }
    }, 100);
};
